- [1 基础知识](#1-基础知识)
  ...
  - [1.6 饿汉模式 （程序启动时创建并初始化对象）](#16-饿汉模式-程序启动时创建并初始化对象)
    - [（1）【全局静态变量（类内声明，类外定义对象）】实现饿汉模式](#1全局静态变量类内声明类外定义对象实现饿汉模式)
  - [1.7 懒汉模式（需要时创建并初始化对象）](#17-懒汉模式需要时创建并初始化对象)
    - [（1）【全局静态指针（类内声明，类外初始化NULL，用时new对象）+ 加锁】实现线程安全懒汉模式](#1全局静态指针类内声明类外初始化null用时new对象-加锁实现线程安全懒汉模式)
    - [（2）【全局静态指针（类内声明，类外初始化NULL，用时new对象）+ 双重检查锁定】实现线程安全懒汉模式](#2全局静态指针类内声明类外初始化null用时new对象-双重检查锁定实现线程安全懒汉模式)
    - [（3）【局部静态变量（用时直接定义对象）】实现线程安全懒汉模式](#3局部静态变量用时直接定义对象实现线程安全懒汉模式)
  - [1.8 信号量（C）](#18-信号量c)
    - [（1）示例](#1示例)
  ...
  - [1.19 std::move](#119-stdmove)
  - [1.20 可调用对象封装器](#120-可调用对象封装器)
    - [（1）std::packaged\_task](#1stdpackaged_task)
    - [（2）std::function](#2stdfunction)
  - [1.21 函数绑定器](#121-函数绑定器)
    - [（1）std::bind](#1stdbind)
    - [（2）和std::function配合使用](#2和stdfunction配合使用)
  - [1.22 别名](#122-别名)
  - [1.23 类静态函数的声明和定义](#123-类静态函数的声明和定义)
  - [1.24 回调函数](#124-回调函数)
  - [1.25 可变参数列表](#125-可变参数列表)
  - [1.26 互斥锁和自旋锁](#126-互斥锁和自旋锁)
  - [1.27 io输入练习](#127-io输入练习)
...
- [2 功能模块](#2-功能模块)
  - [2.1 线程池](#21-线程池)
    ...
    - [（3） 互斥锁，信号量，条件变量的一些思考](#3-互斥锁信号量条件变量的一些思考)
      - [使用场景](#使用场景)
      - [线程池中，线程发生阻塞的条件](#线程池中线程发生阻塞的条件)
...
### （1）【全局静态指针（类内声明，类外初始化NULL，用时new对象）+ 加锁】实现线程安全懒汉模式
```c++
#include <mutex>

class Singleton {
private:
    static Singleton* instance;     // 注意：类内声明，类外初始化NULL，用时new对象
...
### （2）【全局静态指针（类内声明，类外初始化NULL，用时new对象）+ 双重检查锁定】实现线程安全懒汉模式
```c++
#include <mutex>
...
public:
    static Singleton* getInstance() {
        static Singleton instance;  // 用到时直接定义对象
        return &instance;
    }
};
...
## 1.25 可变参数列表
## 1.26 互斥锁和自旋锁
## 1.27 [io输入练习](https://www.nowcoder.com/exam/test/73007554/detail?pid=27976983#question)
### （1）接收一组未知数量的整形数据
```c++
// 接收一行
#include<iostream>
...
### （3） 互斥锁，信号量，条件变量的一些思考
#### 使用场景
    1. 互斥锁是为了实现资源的互斥访问，要求只能有一个线程同时访问资源时，就用互斥锁。例如，线程池中，在对请求队列进行操作之前需要上锁，操作完之后释放锁。
    2. 如果考虑资源的数量有限，可能会被用完，就用信号量或者条件变量。一旦资源用完就使线程阻塞，有资源可用时就唤醒线程。例如，线程池中，管理请求的数量时，当没有请求需要处理就阻塞线程，有了新请求就唤醒线程去处理，可以唤醒一个或全部。
#### 线程池中，线程发生阻塞的条件
    1. 没有获取到互斥锁而阻塞，这种情况只要互斥锁一释放，因等待锁而阻塞的线程会再次去竞争锁，一旦拿到锁就可以继续跑下去，没拿到的继续阻塞。
    2. 因为请求队列为空而阻塞（也即资源用完），线程会阻塞到信号量或条件变量上，当资源可用时必须手动唤醒线程。

    所以只使用互斥锁不能实现线程池，必须搭配信号量或互斥量来管理请求的数量。
